...
(function(parent) {
	...
	if (Logs) {
		logger = console;
		//For Bowser on Android the log does not show in the console only error.
		if (BrowserDetect.browser == "Bowser" && BrowserDetect.OS == "Linux") {
			logger.log = logger.error;
		}
	}

	// WCG URL resources
	var SESSION = "session", REGISTER_RESOURCE = "register", CONFERENCE_RESOURCE = "mediaconf", CONFERENCE_RESOURCE_ADD = "add", CONFERENCE_RESOURCE_REMOVE = "remove", AUDIOVIDEO_RESOURCE = "audiovideo", CHANNEL_RESOURCE = "channels", FILETRANSFER_RESOURCE = "filetransfer", FILETRANSFER_RESOURCE_SEND = "sendfile", FILETRANSFER_RESOURCE_UPLOAD = "uploadfiledata", FILETRANSFER_RESOURCE_ACCEPT = "accept", FILETRANSFER_RESOURCE_TERMINATE = "terminate", FILETRANSFER_RESOURCE_GETFILE = "getfiledata", ADDRESSBOOK_RESOURCE = "ab", ADDRESSBOOK_RESOURCE_CONTACTS = "contacts", CHAT_RESOURCE = "message", CHAT_RESOURCE_SEND = "send", CHAT_RESOURCE_SEND_ISCOMPOSING = "send-iscomposing", CHAT_RESOURCE_SEND_MEDIA = "sendmedia?to=", CHAT_RESOURCE_GET_MEDIA = "getmedia?fileRef=", GROUP_CHAT_RESOURCE = "groupchat", GROUP_CHAT_CREATE = "create", GROUP_CHAT_LEAVE = "leave", GROUP_CHAT_ADD = "addparticipants", GROUP_CHAT_JOIN = "join", GROUP_CHAT_ACCEPT = "invite/accept", GROUP_CHAT_DECLINE = "invite/decline", GROUP_CHAT_RESOURCE_SEND_MEDIA = "sendmedia?confId=";
	PRESENCE_RESOURCE = "presence", PRESENCE_RESOURCE_USER = "user", PRESENCE_RESOURCE_USER_SUBSCRIBE = "subscribe", PRESENCE_RESOURCE_USER_PUBLISH = "publish", PRESENCE_RESOURCE_USER_ANONYMOUS = "anonymous", PRESENCE_RESOURCE_LIST = "list", PRESENCE_RESOURCE_LIST_ADD = "adduser", PRESENCE_RESOURCE_LIST_REMOVE = "removeuser", PRESENCE_RESOURCE_LIST_BLOCK = "blockuser", CONTENT_RESOURCE = "content", CONTENT_RESOURCE_GETAVATAR = "getavatar", CONTENT_RESOURCE_SETAVATAR = "setavatar", CONTENT_RESOURCE_DELETEAVATAR = "deleteavatar";
	...
	MediaServices = function(gwUrl, username, authentication, services) {
		...
		//this._accessToken = _Base64encode(this._accessToken);
		//console.log("base64 ACCESS TOKEN", this._accessToken);
			...
			if (this.readyState == 4) {
				...
				if (this.status == 201) {
					...
					/*if (mediaService._isSipUser) {
					 ...
					 setTimeout(function(){mediaService._publishServices();},5);
					 }*/
	...
	MediaServices.prototype.anonymousSubscribe = function() {
		...
		var url = this._gwUrl + PRESENCE_RESOURCE + '/' + PRESENCE_RESOURCE_USER + '/' + PRESENCE_RESOURCE_USER_ANONYMOUS + '/' + PRESENCE_RESOURCE_USER_SUBSCRIBE;
	...
	MediaServices.prototype.setAvatar = function(avatar, callback) {
		...
		// Chrome 7+, Firefox 4+, Internet Explorer 10+, Safari 5+
	...
	MediaServices.prototype.createCall = function(recipient, mediaType) {
		...
		logger.log("MediaServices.prototype.createCall.... on " + BrowserDetect.browser);
	 ...
	 */
	MediaServices.prototype.onready = function(evt) {
	};
	// The MediaServices object is ready to use
	 ...
	 */
	MediaServices.prototype.onerror = function(evt) {
	};
	// An error occurred. See the event for details.
	 ...
	 */
	MediaServices.prototype.onclose = function(evt) {
	};
	// The session has ended and this MediaServices object is no longer available to use
	 ...
	 */
	MediaServices.prototype.onstatechange = function(evt) {
	};
	// The MediaServices object has changed state
	 ...
	 */
	MediaServices.prototype.oninstantmessage = function(evt) {
	};
	// The MediaServices object is ready to use
	 ...
	 */
	MediaServices.prototype.oninvite = function(evt) {
	};
	// An invitation to a call/conference has been received
	...
	Call = function(mediaServices, recipient, mediaType) {
				...
				switch (newState) {
					case Call.State.RINGING:
						if (this instanceof IncomingCall) {
							...
							if ( typeof (mediaServices.oninvite) == "function") {
								mediaServices.oninvite(evt);
							}
						}
						break;
		...
		Object.defineProperty(this, "remoteStreams", {
			get : function() {
				if (this._pc) {
					return (this._pc.getRemoteStreams ? this._pc.getRemoteStreams() : this._pc.remoteStreams);
				} else {
					return [];
				}
			}
		...
		Object.defineProperty(this, "localStreams", {
			get : function() {
				if (this._pc) {
					return (this._pc.getLocalStreams ? this._pc.getLocalStreams() : this._pc.localStreams);
				} else {
					return [];
				}
			}
		...
		this.isDropVideo = false;

		/**
		 Is it an initial UPGRADE VIDEO offer
		 @private
		 */
		this._isModVideoOffer = false;

		/**
		 Is it an initial UPGRADE VIDEO offer
		 @private
		 */
		this._isModVideoAccept = false;
	...
	Call.prototype.isConference = function() {
		return false;
	};
		...
		navigator.webkitGetUserMedia(medias, function(stream) {
			...
			try {
				mt._pc = new webkitDeprecatedPeerConnection(turnConf, function(sig) {
					...
					logger.log("turnConfig: " + turnConf);
				...
				logger.error("Got error with webkitDeprecatedPeerConnection " + e);
				...
				req.onreadystatechange = function() {
					if (req.readyState == 4) {
						...
						if (req.status == 202) {
							...
							_call.state = Call.State.RINGING;
							var event = {
								success : true,
								failure : false
							};
		...
		req.onreadystatechange = function() {
			if (req.readyState == 4) {
				...
				if (req.status == 204) {
					...
					logger.log("Leave call unsuccessful: " + reason);
	...
	function _updateSdp(sdp, oldString, newString) {
		var indexof = sdp.lastIndexOf(oldString);
		while (indexof > 0) {
			var prefix = sdp.substr(0, indexof);
			var remaining = sdp.substr(indexof + oldString.length);
			sdp = prefix + newString + remaining;
			indexof = sdp.lastIndexOf(oldString);
		}
		return sdp;
	};
	...
	/**
	 Detach Video from Audio/Video Call
	 */
	Call.prototype.dropVideo = function() {
		this.isDropVideo = true;
		this._modify("dropvideo");
	};

	/**
	 Attach Video to Audio Call
	 */

	Call.prototype.addVideo = function() {
		this._isSignalingSent = false;
		var current = this;
		navigator.webkitGetUserMedia({
			...
			current._pc.removeStream(current._pc.localStreams[0]);
			current._pc.addStream(stream);
			current._modify("addvideo");
		}, function(error) {
			logger.log("Error obtaining user media: " + error.toString());
			var callType = ( currentCall instanceof Conference) ? Conference : Call;
			_InternalError(currentCall, callType.Error.USER_MEDIA);
		});
	};
	...
	Call.prototype._modify = function(action) {
		if (action == "addvideo") {
			...
			this._isModVideoOffer = true;
		} else if (action == "dropvideo") {
		...
		if (BrowserDetect.intversion < 25)
			constraints = this.mediaType;
		else
			constraints = {
				'mandatory' : {
					'OfferToReceiveAudio' : !!this.mediaType.audio,
					'OfferToReceiveVideo' : !!this.mediaType.video
				}
			};

		_call._pc.createOffer(function(sdp) {
			logger.log("Mod SDP offer from Browser = " + sdp.sdp);
			_call._sdp.sdp = sdp.sdp;

			_call._isModifModerator = true;
			var req = new _CreateXmlHttpReq();
			var newsdp = _call._sdp.sdp;

			if (action == "resume") {
				newsdp = _updateSdp(_call._sdp.sdp, "sendonly", "sendrecv");
			} else if (action == "hold") {
				newsdp = _updateSdp(_call._sdp.sdp, "sendrecv", "sendonly");
			}
			...
			var modifyLocalDescriptor = new RTCSessionDescription(descriptor, function() {
				logger.log("Success Local RTCSessionDescription");
			}, function(err) {
				logger.log("Err Local RTCSessionDescription " + err);
			});

			_call._pc.setLocalDescription(modifyLocalDescriptor);
			if (action == "addvideo") {
				if (_call._pc.localStreams[0])
					logger.log("waiting for video candidates");
				return;
			}
				...
				if (req.readyState == 4) {
					...
					if (req.status >= 200 && req.status <= 204) {
						if (action != "addvideo") {
							...
							logger.log("mod happening no state change");
						}
					} else {
						var json = JSON.parse(req.responseText);
	...
	function delayTransferBecauseOfMTASBug(_call, transferToCall, event) {
		logger.log("Performing transfer from " + _call._callID + " to " + transferToCall._callID);
	...
	/**
	 ...
	 @example
	 call.transferto(sip:someone@domain.com);
	 */
	Call.prototype.transferto = function(transferaddress) {

		var _call = this;
		if (_call.state != Call.State.HOLDING) {
			logger.log("Call is not on hold cannot transfer.");
		}

		//Make second call to transferaddress
		transferToCall = _call._mediaServices._createTransferCall(transferaddress);
		transferToCall.onaddstream = function(event) {
			logger.log("Got transferTarget media stream");
		};
		transferToCall.onbegin = function(event) {
			logger.log("TransferTarget connected");
			setTimeout(delayTransferBecauseOfMTASBug, 6000, _call, transferToCall, event);
		};
		transferToCall.onend = function(event) {
			logger.log("TransferTarget call has ended.");
		};
		transferToCall.onerror = function(event) {
			logger.log("TransferTarget call error ending transfer.");
			transferToCall.end();
		};
		transferToCall.onremovestream = function(event) {
		};
		transferToCall.onstatechange = function(event) {
			logger.log("TransferTarget new state " + transferToCall.getStringState());
		};
		transferToCall.ring();
	};
	...
	function sendSdp(mt) {
		if (!mt._isSignalingSent) {
			...
			if (mt.isMozillaConn) {
				logger.log("SDP answer from Browser = " + mt._sdp.sdp);
				// epetstr mt._pc.localDescription is null in FF
				mt._sendSignaling(mt._sdp.type, mt._sdp.sdp);
			} else {
				logger.log("SDP answer from Browser = " + mt._pc.localDescription.sdp);
				if (mt.mediaType.video || mt._isModVideoAccept != false) {
	...
	/**
	 ...
	 @private
	 */
	Call.prototype._createChromePeerConnection = function(callback) {
		var mt = this;
		logger.log("in _createChromePeerConnection " + callback);

		// Get the user's media
		navigator.webkitGetUserMedia(mt.mediaType, function(stream) {
			try {
				logger.log("Turnconfig: " + mt._mediaServices.turnConfig);
				//var pc_config = {"iceServers": [{"url": mt._mediaServices.turnConfig}]};
				pc_config = {};
				pc_config.iceServers = [{
					"url" : mt._mediaServices.turnConfig,
					"credentials" : null
				}];
				...
				mt._pc.onicecandidate = function(event) {
					...
					if (event.candidate != null) {
						logger.log("onicecandidate called: " + JSON.stringify(event.candidate));
						nCand++;
						logger.log("nr of gathered candidates: " + nCand);
						if (sendTimer == -1)
							sendTimer = setTimeout(sendSdp, 5000, mt);
				...
				logger.log("catched exception in webkitGetUserMedia callback function: " + e.message);
			...
			mt._pc.onconnecting = function() {
				logger.log("peer connecting !");
			};
			mt._pc.onopen = function() {
				if (mt.state != Call.State.WAITING && mt.state != Call.State.HOLDING && mt.state != Call.State.TRANSITION)
					mt.state = Call.State.ONGOING;
			};
	...
	Call._userMediaError = function(e) {
		if ( typeof e == typeof {}) {
			logger.log("Error in call setup" + ": " + JSON.stringify(e));
			//logger.log(contextmsg + ": " + JSON.stringify(e));
		} else {
			logger.log("Error in call setup" + ": " + e);
			//logger.log(contextmsg + ": " + e);
		}
	...
	Call.prototype._createMozPeerConnectionHelper = function(callback, audiostream, videostream) {
		var mt = this;
		// I know mt variable not needed in this helper function but stil convenient due to similarity to chrome createPerConnection
		logger.log("Turnconfig: " + mt._mediaServices.turnConfig);
		...
		logger.log("pc_config:" + JSON.stringify(pc_config));
		...
		mt._pc.onicecandidate = function(event) {
			logger.log("pc.onicecandidate called");
		};

		// Propagate the event
		mt._pc.onaddstream = function(evt) {
			logger.log("pc.onaddstream called " + evt.type);
			if ( typeof (mt.onaddstream) == "function") {
				//evt.call = mt;
				logger.log("found pc.onaddstream callback " + evt.type);
				mt.onaddstream(evt);
			}
		...
		mt._pc.onstatechange = function(obj) {
			logger.log("onstatechange with " + obj);
		};

		mt._pc.ongatheringchange = function(obj) {
			logger.log("ongatheringchange with " + obj);
		};

		mt._pc.onicechange = function(obj) {
			logger.log("onicechange with " + obj);
		};
		...
		// MOZFIX mt._pc.onconnecting = function() { logger.log("peer connecting !"); };
		mt._pc.onopen = function() {
			if (mt.state != Call.State.WAITING)
				mt.state = Call.State.ONGOING;
		};
	...
	Call.prototype._sendSignaling = function(type, sdp) {
		var _call = this;
		var url = this._url;
		logger.log("Sending " + type + " with SDP: " + sdp);
		if ((type == "OFFER" || type == "offer") && (_call._isModVideoAccept != true ) || _call._isModVideoOffer) {
			if (this._modID != null) {
				logger.log("Sending MOD OFFER");
				url += "/mod/" + this._modID;
			} else if (this._isModVideoOffer) {
				logger.log("Sending init mod offer");
				url += "/mod";
			} else {
			...
			if (this instanceof Conference && !this.confID) {
				...
				// Audio video invite
				var body;
				if (_call.isMozillaConn) {
					body = _MozParseSDP(this.recipient, sdp);
				} else {
					body = _ParseSDP(this.recipient, sdp);
				}
				...
				req.onreadystatechange = function() {
					if (req.readyState == 4) {
						...
						if (req.status >= 200 && req.status <= 204) {
							...
							if (_call._isModVideoAccept != true && _call._isModVideoOffer != true) {
								logger.log("Audio video invite: " + json.state);
								_call.state = Call.State.RINGING;
							} else {
								_call._isModVideoAccept = false;
								_call._isModVideoOffer = false;
								logger.log("Sending a video upgrade/downgrade offer/answer");
							}
						} else {
		...
		} else if (type == "ANSWER" || type == "answer" || this._isModVideoAccept) {
			if (this._modID != null) {
			...
			var body;
			if (_call.isMozillaConn) {
				body = _MozParseSDP(null, sdp);
			} else {
				body = _ParseSDP(null, sdp);
			}
	...
	Call.prototype._doOffer = function() {
		...
		if (this.isRTCConnection) {
			...
			this._pc.createOffer(function(sdp) {
				logger.log("SDP offer from Browser = " + sdp.sdp);
				mt._sdp.sdp = sdp.sdp;
				mt._sdp.type = "OFFER";
	...
	Call.prototype._doMozOffer = function() {
		...
		// success
		function(offer) {
			logger.log("_doMozOffer createOffer: " + offer.sdp);
			...
			if (navigator.userAgent == "Nightly-Leif") {
				...
				logger.log("FF-Nightly modified sdp:" + mod_sdp);
				offer.sdp = mod_sdp;
			}
			...
			logger.log("before set localDescription");
			mt._pc.setLocalDescription(offer,
			// success
			function() {
				logger.log("This offer is set as localDescription");
				logger.log("send sdp offer");
				sendSdp(mt);
			},
			// failure
			function() {
				logger.log("Failed to set this offer as localDescription!");
			}, this.mediaType);
		},
		// fail
		function() {
			logger.log("Failed to create local offer!");
		});
	...
	Call.prototype._doAnswer = function() {
		...
		var mt = this;
		//needed for anonymous function
		logger.log("_doAnswer " + this.mediaType);
		if (this.isRTCConnection) {
			try {
				var remoteSdp = {};
				...
				var remoteDesc = new RTCSessionDescription(remoteSdp, function() {
					logger.log("Success Remote RTCSessionDescription");
				}, function(err) {
					logger.log("THAO 1");
					logger.log("Err Remote RTCSessionDescription " + err);
				});

				mt._pc.setRemoteDescription(remoteDesc, function() {//success
					var constraints;
					if (BrowserDetect.intversion < 25)
						constraints = mt.mediaType;
					else
						constraints = {
							'mandatory' : {
								'OfferToReceiveAudio' : !!mt.mediaType.audio,
								'OfferToReceiveVideo' : !!mt.mediaType.video
							}
						};
					mt._pc.createAnswer(function(sdp) {
						logger.log("SDP answer from Browser = " + sdp.sdp);
						if (mt._sdp.sdp.indexOf("sendonly") != -1) {//remote has sendonly
							var newsdp = _updateSdp(sdp.sdp, "sendrecv", "recvonly");
							sdp.sdp = newsdp;
						}

						if (mt._modID != null /*&& mt._isModVideoAccept !=true*/) {
							if (mt._sdp.sdp.indexOf("sendonly") != -1) {//remote has sendonly
								mt.state = Call.State.WAITING;
							} else {
								mt.state = Call.State.ONGOING;
								if (mt._sdp.sdp.indexOf("sendrecv") != -1 && sdp.sdp.indexOf("recvonly") != -1) {
									var newsdp = _updateSdp(sdp.sdp, "recvonly", "sendrecv");
									sdp.sdp = newsdp;
								}
							}
						}
						mt._sdp.sdp = sdp.sdp;
						mt._pc.setLocalDescription(sdp);
						if (mt._isModVideoAccept != true) {
							if (mt._modID != null) {
								mt._sendSignaling("ANSWER", sdp.sdp);
							}
						} else {
							mt.mediaType.video = true;
						}
					}, function(err) {
						logger.error("Err create answer " + err);
					}, constraints);
				}, function(err) {
					logger.error("Err setRemoteDescription " + err);
				});
			} catch(e) {
	...
	Call.prototype._doMozAnswer = function() {
		...
		var mt = this;
		//needed for anonymous function
		logger.log("_doMozAnswer " + this.mediaType);
		...
		mt._sdp.type = 'answer';
		mt._candidates.length = 0;
		logger.log("offer sdp " + remoteSdp.sdp);

		mt._pc.setRemoteDescription(remoteSdp, function() {// success
			mt._pc.createAnswer(function(sdp) {

				logger.log("SDP answer= " + sdp.sdp);
				mt._sendSignaling("ANSWER", sdp.sdp);

				mt._pc.setLocalDescription(sdp,
				// success
				function() {
					logger.log("succeed: answer setLocalDescription");
				},
				// failure
				function() {
					logger.log("fail: answer setLocalDescription");
				});
				mt._sdp.sdp = sdp.sdp;
			}, function(err) {// fail
				logger.log("fail: create answer " + err);
			}, mt.mediaType);
		}, function(err) {
			logger.log("Err setRemoteDescription " + err);
		});
	};
	...
	Call.prototype.ondropvideo = function() {
	};
	/**
	...
	OutgoingCall.prototype.ring = function() {
		...
		} else if (navigator.webkitGetUserMedia && BrowserDetect.browser == "Chrome") {
			logger.log("This appears to be Chrome");
			this._createChromePeerConnection(function() {
				call._doOffer();
			});
		} else if (BrowserDetect.browser == "Bowser") {
			logger.log("This appears to be Bowser");
	 ...
	 */
	IncomingCall = function(mediaServices, recipient, mediaType, resourceURL) {
		// call parent constructor
		var call = Call.prototype.constructor.call(this, mediaServices, recipient, mediaType);
		call._isModerator = false;
		//thao
		call._url = mediaServices._host + resourceURL;
		/**
		 Remote SDP
		 @deprecated Not used for JSEP. To remove when ROAP is no longer supported.
		 @private
	...
	IncomingCall.prototype.answer = function() {
		...
		if (navigator.mozGetUserMedia) {
			...
			try {
				...
				console.error("exception in _doMozAnswer" + e.message);
			}
	...
	Conference.prototype.isConference = function() {
		return true;
	};
	/**
	...
	Conference.prototype.join = function() {
		...
		if (navigator.mozGetUserMedia) {
			logger.log("This appears to be Firefox");
			console.error("Media conference missing implementation for Firefox.");
			this.isMozillaConn = true;
		} else if (navigator.webkitGetUserMedia && BrowserDetect.browser == "Chrome") {
			logger.log("This appears to be Chrome");
			this._createChromePeerConnection(function() {
			...
			set : function(newState) {
				...
				switch (newState) {
					...
					case FileTransfer.State.DOWNLOAD_COMPLETE:
						...
						if ( typeof (this.onreceivedfile) == "function") {
							this.onreceivedfile(evt);
						}
					default:
						break;
				}
	...
	FileTransfer.prototype.cancel = function() {
		...
		if (!this._id || this.state == FileTransfer.State.UPLOAD_COMPLETE || this.state == FileTransfer.State.DOWNLOAD_COMPLETE) {
	...
	/**
	 ...
	 @example
	 ftp.onuploadprogress = function(evt) {
	 logger.log(event.loaded / event.total * 100 + "%");
	 };
	 */
	...
	IncomingFileTransfer.prototype.accept = function() {
		if (this.state == FileTransfer.State.UPLOAD_COMPLETE || this.state == FileTransfer.State.DOWNLOAD_COMPLETE) {
			throw new Error("No active file transfer session");
		}
	...
	GroupChat = function(mediaServices, subject, recipients, owner, confID) {
		var _subject = subject, _confID = confID;
	...
	ContactList.prototype._parseAddressBook = function(contacts) {
		...
		var previousContacts = this._contacts.splice(0, this._contacts.length);
		for (var i in contacts) {
			try {
				var info = contacts[i];
				var contact = new Contact(info.vcard);
		...
		this._updatePresenceInfo(previousContacts);
	...
	/**
	 ...
	 @private
	 */
	ContactList.prototype._updatePresenceInfo = function(previousContactsList) {
		for (var i in this._contacts) {
			var contact = this._contacts[i];
			for (var j in previousContactsList) {
				if (previousContactsList[j]._id == contact._id) {
					contact._avatar = previousContactsList[j]._avatar;
					contact._presence = previousContactsList[j]._presence;
					contact._services = previousContactsList[j]._services;
					contact._presenceState = previousContactsList[j]._presenceState;
					break;
				}
			}
		}
					...
					if (this.contact[j]._id == user) {
						...
						if (presence.person.statusIconEtag == "0") {
							...
							if (!this.contact[j].avatar || presence.person.statusIconEtag != this.contact[j].presence.statusIconEtag) {
								...
								this.contact[j].presence.statusIconUrl = presence.person.statusIconUrl.substring(presence.person.statusIconUrl.indexOf("content/getavatar/"), presence.person.statusIconUrl.length);
	...
	Contact = function(info) {
		var _state = Contact.State.NEW, _name = null, _numbers = [], _emails = [], _addresses = [], _note = "", _org = "", _title = "", _birthday = "", _photo = "", _presence = null, // RCS Presence attributes for this contact
		_services = null;
	...
	function _Channel(mediaService) {
		var timer = 2000, channel = this, _ms = mediaService;
				...
				if (this.readyState == 4) {
					...
					if (this.status == 200) {
						...
						for (var i = 0; ; i++) {
							...
							var type = eventObject["@type"], state = eventObject.state, from = eventObject.from;

							// Channel Handlers
							if (type == "audiovideo" || type == "media-conference") {
								var sdp = eventObject.sdp, resourceURL = eventObject.resourceURL, reason = eventObject.reason;
								...
								if (state.toLowerCase() == "session-open" || state.toLowerCase() == "session-modified") {
									...
									if (mediaConfIndex != -1) {
										...
										if (_ms._call._callID == null || _ms._call._callID == callId) {
											...
											logger.log("What to do with call id: " + callId + " ?? ");
										}
									...
									try {
										if (window.webkitDeprecatedPeerConnection && (currentCall._pc instanceof webkitDeprecatedPeerConnection)) {
											logger.log("Opening Deprecated peer connection.");
											if (currentCall._isModerator || ( currentCall instanceof Conference && !currentCall._isModerator)) {
												var roapMessage = currentCall._DEPRECATEDroap.processRoapAnswer(_ms, sdp, eventObject.sdpSessionOrigin, eventObject.attributes);
											...
											if (currentCall._isModerator || ( currentCall instanceof Conference && !currentCall._isModerator)) {
												...
												if (currentCall.isMozillaConn) {
													remoteDest.sdp = _MozSDPToString(eventObject.sdpSessionOrigin, sdp, eventObject.attributes);
													//logger.log("mozilla answer sdp:"+remoteDest.sdp);
													//remoteDest.sdp = remoteDest.sdp.replace(/mslabel:/g, "mslabel:2"); //Hack but cause the hold scenario to fail.
													//logger.log("Modified SDP " + remoteDest.sdp);
													// Receive ANSWER SDP of callee
													currentCall._pc.setRemoteDescription(remoteDest, function() {
														logger.log("Success setRemoteDescription ");
													}, function(err) {
														logger.log("Err setRemoteDescription " + err);
													});

												} else if (currentCall.isRTCConnection) {
													logger.log("Chrome answer sdp = " + sdp );
													remoteDest.sdp = _SDPToString(eventObject.sdpSessionOrigin, sdp, eventObject.attributes);
													logger.log("Chrome answer sdp AFTER = " + remoteDest.sdp );
													//logger.log("Chrome answer sdp = " + remoteDest.sdp );
													//remoteDest.sdp = remoteDest.sdp.replace(/mslabel:/g, "mslabel:2"); //Hack but cause the hold scenario to fail.
													//logger.log("Modified SDP " + remoteDest.sdp);
													var rtcSessionDesc = new RTCSessionDescription(remoteDest);
													currentCall._pc.setRemoteDescription(rtcSessionDesc, function() {
														logger.log("Success Remote RTCSessionDescription");
													}, function(err) {
														logger.log("THAO 2");
														logger.log("Err Remote RTCSessionDescription " + err);
									...
									logger.log("MediaServices call invitation receive.... on " + BrowserDetect.browser);
									...
									//thao: workaround. Need to pass the resource URL before creating the instance. Otherwise,
									//the receiving party would not be able to reject the call (invalid URL)
									_ms._call = new IncomingCall(_ms, from, mediaType, eventObject.resourceURL);
									...
									if (navigator.mozGetUserMedia) {
										_ms._call._sdp.sdp = _MozSDPToString(eventObject.sdpSessionOrigin, sdp, eventObject.attributes);
									} else {
									...
									sdp.origin = eventObject.sdpSessionOrigin;
									sdp.attributes = eventObject.attributes;
									...
									var remoteSdp;
									if (navigator.mozGetUserMedia) {
										remoteSdp = _MozSDPToString(eventObject.sdpSessionOrigin, sdp, eventObject.attributes);
									} else {
										remoteSdp = _SDPToString(eventObject.sdpSessionOrigin, sdp, eventObject.attributes);
									}

									_ms._call._sdp.sdp = remoteSdp;
									...
									if (_ms._call._sdp.sdp.indexOf("recvonly") != -1) {
										...
										logger.log("Hold call successful...other party is waiting.");

									} else if (_ms._call._sdp.sdp.indexOf("sendonly") != -1) {
										_ms._call.state = Call.State.WAITING;
										logger.log("Hold call successful...waiting for other party to resume.");
										...
										logger.log("Resume call successful...call ongoing.");
									}

									if (currentCall.isDropVideo) {
										currentCall.ondropvideo();
										currentCall.isDropVideo = false;
									}
									...
									if (currentCall.isRTCConnection && currentCall._isModifModerator) {
										...
										currentCall._pc.setRemoteDescription(new RTCSessionDescription(remoteDest), function() {
											logger.log("Success Remote RTCSessionDescription");
										}, function(err) {
											logger.log("THAO 3");
											logger.log("Err Remote RTCSessionDescription " + err);
									...
									if (remoteSdp.indexOf("m=video") != -1 && (currentCall._sdp.sdp.indexOf("m=video") == -1 || currentCall._sdp.sdp.indexOf("c=IN IP4 0.0.0.0") != -1 ) && remoteSdp.indexOf("sendrecv") != -1) {
										currentCall._isModVideoAccept = true;
										currentCall.mediaType.video = true;
									}
									if ((remoteSdp.indexOf("m=video") === -1 && _ms._call._sdp.sdp.indexOf("m=video") != -1) && remoteSdp.indexOf("sendrecv") != -1) {
										//detach video
										currentCall.isDropVideo = true;
										currentCall.mediaType.video = false;
									}
									currentCall._sdp.sdp = remoteSdp;

									if (currentCall._sdp.sdp.indexOf("sendonly") != -1) {
										currentCall.state = Call.State.TRANSITION;
									}

									logger.log("Processing modification: " + currentCall._modID);

									currentCall._isModifModerator = false;
									//answer
									if (currentCall._isModVideoAccept != false) {
										navigator.webkitGetUserMedia({
											audio : true,
											video : true
										}, function(stream) {
											currentCall._isSignalingSent = false;
											sendTimer = -1;
											currentCall._pc.removeStream(currentCall._pc.localStreams[0]);
											currentCall._pc.addStream(stream);
											if (currentCall.isMozillaConn) {
												logger.log("Generate Moz Mod answer");
												currentCall._doMozAnswer();
											} else {
									...
									logger.log("transfer-terminated");
								...
								var fileName = eventObject.fileName, fileSize = eventObject.fileSize, contentType = eventObject.contentType, ftId = eventObject.ftId;
								...
								if (state.toLowerCase() == "session-open" && ft) {
									...
									if (ft.state != FileTransfer.State.ERROR && ft.state != FileTransfer.State.CANCELED) {
								...
								var referredBy = eventObject.referredBy, confId = eventObject.confId, subject = eventObject.subject, members = eventObject.members;
								...
								var userCount = eventObject.userCount, confId = eventObject.confId,
								...
								if (ongoingGroupChat) {
									if (userCount == 0 || users.length == 0) {
										...
										if (ongoingGroupChat.state != GroupChat.State.ENDED) {
											...
											if (ongoingGroupChat.state != GroupChat.State.ENDED) {
												ongoingGroupChat.onupdate({
													members : ongoingGroupChat.members
												});
											}
										} else {
								...
								var from = eventObject.from, confId = eventObject.confId, body = eventObject.body;
								...
								var from = eventObject.from, confId = eventObject.confId,
								//contentType = eventObject.contentType,
								//size = eventObject.size,
								url = eventObject.url;
								...
								var from = eventObject.from, confId = eventObject.confId, refresh = eventObject.refresh, state = eventObject.state;
					...
					// Error response //128000
					else {
						if (timer > 64000) {
							// Change state to ERROR since channel hasn't responded for 4 minutes
							logger.log("change of state to ERROR");
							_ms.state = MediaServices.State.ERROR;
						} else {
							// If we are unable to poll the channel, attempt to poll for 4 minutes total exponentially, then stop
							// 4s + 8s + 16s + 32s + 64s + 128s = 252s
							if (_ms._channel != null) {
								timer *= 2;
	...
	function _MozParseSDP(recipient, sdp) {
		...
		// add attribute mozpatch to audio media section is used as workaround to store session-level attributes
		var mozPatch = "";
		logger.log("Building moz generic attributes patch...");
		var lines_array = commonPart.split("\\r\\n");
		for (var i in lines_array) {
			var line = lines_array[i];

			if (line[0] == "a") {
				mozPatch += "|" + line.substring(2);
			}
		}
		mozPatch = "mozPatch-o" + mozPatch;
		mozPatch = {
			a : mozPatch
		};
		logger.log("patch= " + mozPatch);

		var media_line_array = mediaPart.split("m=");
		for (var index in media_line_array) {
			if (media_line_array[index] === "")
				continue;
			...
			if (media_line_array[index].indexOf("audio") == 0) {
				m_struct.attributes.push(mozPatch);
			}
	...
	function _MozSDPToString(origin, sdp, attributes) {
		...
		logger.log("Parsing moz generic attributes patch...");
		if (sdp[0].m.indexOf("audio") == 0) {
			logger.log("Found audio");
			...
			for (attributeIndex in sdp[0].attributes) {
				...
				if (prefix == "mozpatch-o|") {
					logger.log("Found patch");
					var atts = value.substr(11).split('|');
					for (a in atts) {
						session_attribues_str += "a=" + atts[a] + "\r\n";
					}
				}
			}
			//logger.log("Adding attributes: " + session_attribues_str);
		...
		for (var mediaIndex in sdp) {
			...
			for (attributeIndex in sdp[mediaIndex].attributes) {
				//logger.log("Adding media attributes... ");
				if ( typeof (sdp[mediaIndex].attributes[attributeIndex].a) === "undefined" || sdp[mediaIndex].attributes[attributeIndex].a.indexOf("mozpatch-o") == 0)
					continue;
				sdp_str += "a=" + sdp[mediaIndex].attributes[attributeIndex].a + "\r\n";
			}
		}
		logger.log("SDP parsed: " + sdp_str);
		return sdp_str;
	}
	...
	function _SDPToString(origin, sdp, attributes) {
		...
		var genericStreamDirection = "inactive";

		if ( typeof (attributes) != "undefined" && attributes != "") {
			if (attributes.indexOf("sendrecv") > 0) {
				genericStreamDirection = "sendrecv";
			} else if (attributes.indexOf("sendonly") > 0) {
				genericStreamDirection = "sendonly";
			} else if (attributes.indexOf("recvonly") > 0) {
				genericStreamDirection = "recvonly";
			}
			wcgSdp += attributes + "\r\n";
		}
		...
		for (mediaIndex in sdp) {
			...
			var foundMediaStreamDirection = false;
			for (attributeIndex in sdp[mediaIndex].attributes) {
				wcgSdp += "a=" + sdp[mediaIndex].attributes[attributeIndex].a + "\r\n";
				if (!foundMediaStreamDirection && (sdp[mediaIndex].attributes[attributeIndex].a === "sendrecv" || sdp[mediaIndex].attributes[attributeIndex].a === "sendonly" || sdp[mediaIndex].attributes[attributeIndex].a === "recvonly")) {
					foundMediaStreamDirection = true;
				}
			}

			if (!foundMediaStreamDirection && genericStreamDirection != "inactive") {
				wcgSdp += "a=" + genericStreamDirection + "\r\n";
			}
			//THAO
			//wcgSdp += "a=" + "sendrecv" + "\r\n";
		}
		...
		//thao
		logger.log("SDP AFTER MOZILLA PARSING", wcgSdp);
	...
	function _DEPRECATEDRoap() {
		...
		var _H2SRoap = {
			...
			reset : function() {
				messageType = "", SDP = [], offererSessionId = "", answererSessionId = "";
			}
		};
		...
		this.processRoapOffer = function(mediaServices, sdp) {
			...
			var wcgSdp;
			if ( typeof (sdp.origin) === "undefined") {
				wcgSdp = "v=0\r\no=- 0 0 IN IP4 127.0.0.1\r\ns=\r\nt=0 0\r\n";
			} else {
				wcgSdp = "v=0\r\n" + sdp.origin + "\r\ns=\r\nt=0 0\r\n";
			}

			if ( typeof (sdp.attributes) != "undefined" && sdp.attributes != "") {
				wcgSdp += sdp.attributes + "\r\n";
			}

			for (mediaIndex in sdp) {
				if (mediaIndex == "origin" || mediaIndex == "attributes")
					continue;
				wcgSdp += "m=" + sdp[mediaIndex].m + "\r\n";
				wcgSdp += "c=" + sdp[mediaIndex].c + "\r\n";
				for (attributeIndex in sdp[mediaIndex].attributes) {
		...
		this.processRoapAnswer = function(mediaServices, sdp, origin, attributes) {
			...
			var roapMessage = "SDP\n" + JSON.stringify(roapStruct);
			// nicklas fulhack
			roapMessage = roapMessage.replace("UDP 1", "udp 1");
			roapMessage = roapMessage.replace("UDP 1", "udp 1");
			roapMessage = roapMessage.replace("UDP 1", "udp 1");
			roapMessage = roapMessage.replace("UDP 1", "udp 1");

			//roapMessage = roapMessage.replace(/mslabel:/g, "mslabel:2"); //Hack but cause the hold scenario to fail.
			logger.log("tweaked the mslabel");
			// fulhak end
			return roapMessage;
		};
