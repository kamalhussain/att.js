...
call.transferto(transferaddress);
    ...
    */
    Call.prototype._createMozPeerConnectionHelper = function(callback, audiostream, videostream) {
        ...
        pc_config = {};
        //pc_config.iceServers = [{"url": mt._mediaServices.turnConfig, "credentials" : null}];
        pc_config.url = mt._mediaServices.turnConfig;
...
contact.onupdating = function(evt) {
    ...
    */
    ...
    function _MozParseSDP_try1(recipient, sdp) {
        var SDP = {};
        ...
        for(var i in lines_array) {
            ...
            if (line[0] == "v") {
                common_struct.v = line.substring(2);
            }

            if (line[0] == "o") {
                common_struct.o = line.substring(2);
            }

            if (line[0] == "s") {
                common_struct.s = line.substring(2);
            }

            if (line[0] == "t") {
                common_struct.t = line.substring(2);
            }

            if (line[0] == "a") {
                var a_line = { a : line.substring(2) };
                common_struct.attributes.push(a_line);
            }
        }

        // store common part
        SDP.common = common_struct;
    ...
    */
    function _MozSDPToString(origin, sdp) {
        ...
        for (var mediaIndex in sdp) {
            ...
            if (sdp[mediaIndex].m == "application 4711 udp session") {
                // WCG Server ignores v: o: s: t: in m:application section........
                // sdp_str="v="+sdp[mediaIndex].v + "\r\n";
                // if(typeof(origin) === "undefined") {
                    // sdp_str+="o="+sdp[mediaIndex].o + "\r\n";
                // } else {
                    // sdp_str+=origin + "\r\n";
                // }
                // sdp_str+="s="+sdp[mediaIndex].s + "\r\n";
                // sdp_str+="t="+sdp[mediaIndex].t + "\r\n";

                console.log("ignore index 0");
                // moved this handling before for loop due to wcg reorders attributes
                // for(attributeIndex in sdp[mediaIndex].attributes) {
                    // var value = sdp[mediaIndex].attributes[attributeIndex].a;
                    // var prefix = value.substring(0,2);
                    // // check for faked session level v-o-s-t attributes
                    // //"o:Mozilla-SIPUA 27684 0 IN IP4 0.0.0.0"
                    // if (prefix == "v:" || prefix == "s:" || prefix == "t:") {
                        // sdp_str += prefix + value.substring(2) + "\r\n";
                    // } else if ( prefix == "o:") {
                        // // this origin override probably should work
                        // // But for the Mozilla case we want the original value in the offer
    ...
    function _MozSDPToString_try1(origin, sdpObj) {
        // TODO: WCG should return v= o= s= t= a= lines, parse them
        // Hardcode this for now
        ...
        if (sdpObj.common) {
            sdp_str="v="+sdpObj.common.v + "\r\n";
            if(typeof(origin) === "undefined") {
                sdp_str+="o="+sdpObj.common.o + "\r\n";
            } else {
                sdp_str+=origin + "\r\n";
            }
            sdp_str+="s="+sdpObj.common.s + "\r\n";
            sdp_str+="t="+sdpObj.common.t + "\r\n";
            for(attributeIndex in sdpObj.common.attributes) {
                sdp_str += "a=" + sdpObj.common.attributes[attributeIndex].a + "\r\n";
            }
        } else {
            if(typeof(origin) === "undefined") {
                sdp_str = "v=0\r\no=- 0 0 IN IP4 127.0.0.1\r\ns=\r\nt=0 0\r\n";
        ...
        for (mediaIndex in sdpObj.sdp) {
            sdp_str += "m=" + sdpObj.sdp[mediaIndex].m + "\r\n";
            sdp_str += "c=" + sdpObj.sdp[mediaIndex].c + "\r\n";
            for(attributeIndex in sdpObj.sdp[mediaIndex].attributes) {
                sdp_str += "a=" + sdpObj.sdp[mediaIndex].attributes[attributeIndex].a + "\r\n";
            }
        }
